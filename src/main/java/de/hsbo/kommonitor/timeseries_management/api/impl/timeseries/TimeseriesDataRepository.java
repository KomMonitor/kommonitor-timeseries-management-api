...
//	List<TimeseriesDataEntity> findByTimeSeriesIdStartEnd(@Param("timeseries_id") Integer timeseriesId, @Param("start") String start, @Param("end") String end);
	...
	@Query("select t from Timeseries_data as t where t.timeseriesId = :timeseries_id and  (:start is null or t.timestamp > to_timestamp(cast(:start as text), cast('YYYY-MM-dd''T''HH24:mi:ss' as text))) and  (:end is null or t.timestamp < to_timestamp(cast(:end as text), cast('YYYY-MM-dd''T''HH24:mi:ss' as text)))")
	List<TimeseriesDataEntity> findByTimeSeriesIdStartEnd(@Param("timeseries_id") Integer timeseriesId,
			@Param("start") String start, @Param("end") String end);

	@Query("select aggregate_time(:timeseries_id, :interval, :function, to_timestamp(cast(:start as text), cast('YYYY-MM-dd''T''HH24:mi:ss' as text)), to_timestamp(cast(:end as text), cast('YYYY-MM-dd''T''HH24:mi:ss' as text)))")
