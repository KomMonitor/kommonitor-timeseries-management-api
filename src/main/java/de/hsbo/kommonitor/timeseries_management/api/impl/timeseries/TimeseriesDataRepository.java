...
//    @NativeQuery("select time_bucket('1 day'\\:\\:interval, timestamp), avg(value) from Timeseries_data where timeseries_data.timeseries_id = :timeseries_id group by 1")
    @Query("select aggregate_time(:timeseries_id, :interval, :function)")
    String getAggregate(@Param("timeseries_id") Integer timeseriesId, @Param("interval") String interval, @Param("function") String function);
