...
public interface StationsMapper {
	...
	default StationsEntity stationsMetadataToEntity(StationMetadata stationMetadata) {
		...
		List<@Valid Parameter> parametersFromMetadata = stationMetadata.getParameters();
		Set<ParametersEntity> parameterEntities = new HashSet<ParametersEntity>();
		for (Parameter parameter : parametersFromMetadata) {
	...
	StationMetadata stationsEntityToStationMetadata(StationsEntity stationsEntity);
